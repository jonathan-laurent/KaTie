...
let pp_list_inline pp_elem fmt l = 
    let rec aux = function
     ...
     | x::xs -> (fprintf fmt "%a; " pp_elem x ; aux xs) in
     fprintf fmt "[" ;
     aux l ;
     fprintf fmt "]"
...
let pp_option pp_elem fmt = function
  | None -> ()
  | Some x -> pp_elem fmt x
...
let pp_lnk_state f = function
  | Free -> fprintf f "free"
  | Bound_to s -> fprintf f "bound%a" pp_site s
  | Bound_to_any -> fprintf f "bound"
  | Bound_to_type _ -> fprintf f "bound-to-type"

let pp_test f = function
  | Agent_exists aid -> fprintf f "exists(%d)" aid
  | Lnk_state_is (s, st) -> fprintf f "lnk%a = %a" pp_site s pp_lnk_state st
  | Int_state_is (s, st) -> fprintf f "int%a = %a" pp_site s pp_int_state st

let pp_mod f = function
  | Create aid -> fprintf f "add(%d)" aid
  | Destroy aid -> fprintf f "rm(%d)" aid
  | Mod_int_state (s, st) -> fprintf f "int%a := %a" pp_site s pp_int_state st
  | Mod_lnk_state (s, st) -> fprintf f "lnk%a := %a" pp_site s pp_lnk_state st

let pp_measure_descr f = function
  | Event_measure (_, Time) -> fprintf f "time"
...
let pp_event f ev =
  ...
  let print_def_rel msg e ep = 
    ...
    pp_event_pattern f ep
  in
  begin match ev.defining_rel with
    | Some (First_after (e, ep)) -> print_def_rel "First after" e ep
    | Some (Last_before (e, ep)) -> print_def_rel "Last before" e ep
    | None -> ()
  end ;
  pp_newline f
