...
let extract_tests_actions = function
  | Trace.Subs _ -> [], []
  |	Trace.Rule (_, ev, _) -> ev.Instantiation.tests, ev.Instantiation.actions
  | Trace.Pert (_, ev, _) -> ev.Instantiation.tests, ev.Instantiation.actions
  | Trace.Init acts -> [], acts
  | Trace.Obs (_, tests, _) -> tests, []
  | Trace.Dummy _ -> [], []
...
let match_agents_in_pattern
  ...
  if satisfy_rule_constraint pat.rule_constraint w.state w.step then try
    ...
    let compatible_sites (ag_pid, s) (ag, s') =
      compatible_agents ag_pid ag && s = s' in
    ...
    let match_actions pat_action step_action =
      ...
      | Mod_int_state (site, st), Instantiation.Mod_internal (site', st') ->
        if compatible_sites site site' && st = st'
...
let match_simple_pattern
  ...
  | Some ag_matchings -> begin
    ...
    let tests_ok = pat.main_pattern.tests |> List.for_all (fun test ->
      try
        match test with
        | Agent_exists ag_pid ->
          Edges.is_agent (translate_ag ag_pid, p_agent_ty pat ag_pid) prev_mstate
...
let match_event
    ...
    let generate_matchings effective pm1 pm2_opt =
      ...
      if effective then
        { common_to_all = common ; matchings = [{common ; specific}]}
      else
        { common_to_all = common ; matchings = [] } in
