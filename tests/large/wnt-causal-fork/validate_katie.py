...
for ix in global_tracker.keys():
    if (creation_snap_pre.get_agent_from_identifier(ix) is None) and (creation_snap_pst.get_agent_from_identifier(ix) is not None):
...
diff_data_frame = kasim_data.compare(katie_data.round(4), result_names=('KaTIE', 'KaSim'))
if any(diff_data_frame):
    raise ValueError('Data does not match! Diff is\n{}'.format(diff_data_frame))
